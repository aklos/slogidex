...
export default function FormEditor(props: {
  ...
  const updateField = React.useCallback(
    (fieldId: string, key: string, newValue: string) => {
      ...
      (fields as any)[index][key] = newValue;
    ...
    switch (inputType) {
      case "text":
        return (
          <Input
            value={value}
            onChange={(_value: any) => onChange("value", _value)}
          />
        );
      case "number":
        return (
      ...
      case "checkbox":
        return (
          <Checkbox
            checked={value}
            onChange={(_checked: any) => onChange("value", _checked)}
          />
        );
      case "select":
        return (
    ...
    >
      ...
      {inputType !== "checkbox" ? (
        <div className="mb-1 relative">
          <MinimalInput
            value={label}
            onChange={(_value: any) => onChange("label", _value)}
          />
        ...
        <div className="absolute top-1/2 transform -translate-y-1/2 px-2 py-1">
          <label className="flex items-center">
            ...
            <div className="w-48">
              <Input
                value={name}
                onChange={(_value: any) => onChange("name", _value)}
              />
            </div>
          </label>
        </div>
...
function MinimalInput(props: { value: string; onChange: any }) {
